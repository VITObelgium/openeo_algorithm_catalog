"""
Implementation of Soil-Adjusted Vegetation Index (SAVI) for the Copernicus Dataspace Ecosystem Marketplace.
...
def generate() -> dict:
    ...
    red = cube.band(0)
    nir = cube.band(1)
    L = 0.5  # soil brightness correction factor could range from (0 -1)

    # calculate savi
    savi = (nir - red) / (nir + red + L) * (1.0 + L)
    print("SAVI calculation complete.")
...
if __name__ == "__main__":
    with open("savi.json", "w") as f:
        json.dump(generate(), f, indent=2)
    print("UDP 'savi.json' generated.")
